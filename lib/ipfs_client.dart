...
part 'ipfs_exception.dart';
part 'ipfs_logger.dart';
part 'ipfs_method.dart';
part 'ipfs_response.dart';

enum AuthMethod { none, basic, bearer }

class IpfsClient extends IpfsMethod {
  final Uri uri;
  final Client client;
  final String? username;
  final String? password;
  final AuthMethod authMethod;
  final IpfsLogger logger = const IpfsLogger();

  static IpfsClient? _instance;

  static IpfsClient get instance {
    if (_instance == null) {
      throw IpfsException('IpfsClient is not initialized. Call init() first.');
    }
    return _instance!;
  }

  static IpfsClient init({
    required Uri uri,
    String? username,
    String? password,
    ...
    final client = Client();
    _instance ??= IpfsClient._internal(
      client,
      uri: uri,
      username: username,
      password: password,
  ...
  @override
  Future<String> fileCp(String source, String destination) async {
    try {
      logger.info('Copying file from $source to $destination');

      if (source.isEmpty || destination.isEmpty) {
        logger.warning('Source and destination cannot be empty');
        throw IpfsException('Source and destination cannot be empty');
      }

      final requestUri = Request(
        'post',
        uri.replace(
          path: IpfsMethod.filesCpPath,
          queryParameters: {
      ...
      if (_authHeader.isNotEmpty) {
        requestUri.headers[HttpHeaders.authorizationHeader] = _authHeader;
      }

      final response = await client.send(requestUri).then(Response.fromStream);
      ...
      logger.error('Error copying file', e, stackTrace);
      throw IpfsException(
  ...
  Future<PinLsResponse> pinLs({String? cid, String? type}) async {
    try {
      logger.info(
        'Listing pinned objects${cid != null ? ' for CID: $cid' : ''}',
      );
  ...
  Future<StatsResponse> repoStat() async {
    try {
      ...
      logger.info(
        'Repo size: ${statsResponse.repoSize} bytes, Objects: ${statsResponse.numObjects}',
      );
  ...
  Future<List<String>> pubsubLs() async {
    try {
      ...
      final topics =
          (data['Strings'] as List?)?.map((e) => e as String).toList() ?? [];
